import { Connection, Not, Repository, IsNull } from 'typeorm';
...
export class EventsService {
  ...
  async getEventsWithWorkshops() {
    ...
    for (let i = 0; i < events.length; i++) {
      const ws = workshops.filter((w) => w.eventId === events[i].id);
      events[i].workshops = ws;
    }
    return events;
  }
  ...
  async getFutureEventWithWorkshops() {
    ...
    const firstWorkshop = await this.workshopRepository.findOne({
      where: {
        start: Not(IsNull()),
      },
      order: {
    ...
    for (let i = 0; i < events.length; i++) {
      const ws = workshops.filter(
        (w) =>
          w.eventId === events[i].id && w.eventId !== firstWorkshop?.eventId,
      );
      events[i].workshops = ws;
    }
    return events.filter((e: any) => e.workshops.length > 0);
  }
}
